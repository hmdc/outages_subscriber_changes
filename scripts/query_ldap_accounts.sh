...
# disabled users
ldapsearch -LLL -x -o ldif-wrap=no "(&(eduPersonEntitlement=disabled)(uid=*))" gecos mail | grep -Pzo "[a-zA-Z0-9.-]+@[a-zA-Z0-9.-]+\.[a-zA-Z0-9.-]+" | sort | uniq | tr "[:upper:]" "[:lower:]" > /tmp/disabled_rce_users.list